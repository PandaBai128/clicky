...
![Clicky 跟随鼠标、听取问题并指向屏幕目标的卡通桌面场景](docs/assets/clicky-minimax-hero.png)

**一个跟着鼠标走、能看懂当前屏幕，也愿意随时和你聊两句的 Mac 语音伙伴。**

看到陌生的页面、找不到某个按钮，或者只是想弄明白眼前的内容时，按住 `Control + Option` 直接问 Clicky。松开按键后，它会结合当前屏幕回答；如果答案涉及一个看得见的按钮、图标或文件，蓝色指针会飞过去告诉你该看哪里。

## 怎么使用

1. 启动 Clicky，它会待在 Mac 顶部菜单栏，并用蓝色指针跟随鼠标；
2. 按住 `Control + Option` 说出你对当前屏幕的疑问；
3. 松开按键，听取回答，或者跟着蓝色指针找到屏幕上的目标；
...
- 找到可见的按钮、图标、文件或界面区域，并用蓝色指针进行引导；
- 使用语音连续交流，也可以在面板中查看和复制最近的回答；
- 按简短、默认或详细模式调整回答长度；
- 搜索、试听并调整 Clicky 的声音、音量、语速和音调；
- 支持多个显示器，并可在不同屏幕上进行视觉指引。

Clicky 当前负责问答、屏幕解释和视觉指引，不会替你点击、输入或执行脚本。坐标由视觉模型判断，适合作为辅助指引，但不等同于原生 UI 自动化。项目目前也没有内置互联网搜索。
...
1. Clicky 通过腾讯云实时 ASR 将按住说话期间的语音转成文字；
2. App 截取当前屏幕，把问题、对话上下文和截图经代理发送给 MiniMax-M3；
3. 模型生成回答时，Clicky 按句交给 MiniMax TTS 合成并流式播放，完整回答结束后再写入面板历史；
4. 当问题需要寻找可见目标时，模型返回归一化坐标，App 将其映射到对应显示器并移动蓝色指针；
...
- 屏幕与系统录音：截取当前屏幕
- 屏幕内容：在 Clicky 面板中通过系统 Screen Content picker 允许读取可分享屏幕内容
